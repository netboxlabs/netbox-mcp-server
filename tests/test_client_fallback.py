...
def test_fallback_not_triggered_on_non_404_error(client):
    ...
    primary_response.raise_for_status.side_effect = httpx.HTTPStatusError("Server error", request=MagicMock(), response=MagicMock())
...
def test_fallback_not_triggered_on_403_forbidden(client):
    ...
    primary_response.raise_for_status.side_effect = httpx.HTTPStatusError("Forbidden", request=MagicMock(), response=MagicMock())
...
def test_fallback_not_triggered_without_fallback_endpoint(client):
    ...
    primary_response = MagicMock()
    primary_response.status_code = 404
    primary_response.raise_for_status.side_effect = httpx.HTTPStatusError("Not found", request=MagicMock(), response=MagicMock())
...
def test_fallback_error_propagates(client):
    ...
    fallback_response.raise_for_status.side_effect = httpx.HTTPStatusError("Fallback failed", request=MagicMock(), response=MagicMock())
...
def test_both_endpoints_404_propagates_fallback_error(client):
    ...
    fallback_response.raise_for_status.side_effect = httpx.HTTPStatusError("Not found", request=MagicMock(), response=MagicMock())
